...
- **企業のロゴや看板が写り込みやすい場所を指定しない**。航空会社のカウンター、
  店舗の内観、ホテルのフロントなどは、検索結果に他社のロゴが入り込む。
  「ジェットスターの話なのに背景はAlitaliaのカウンター」のような事故が起きる
- 代わりに、特定企業と結びつかない被写体を選ぶ。
  風景、自然、機内から見た雲、滑走路、スーツケース、街並み、温泉、客室の窓辺など。
  例：航空会社の手荷物の話 → "suitcase packing travel"（× "airline check in counter"）
